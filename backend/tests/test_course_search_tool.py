...
class TestCourseSearchTool(unittest.TestCase):
    ...
    def test_execute_successful_search_with_results(self):
        ...
        self.assertEqual(self.search_tool.last_sources[0]["text"], "AI Fundamentals - Lesson 1")
        self.assertEqual(self.search_tool.last_sources[1]["text"], "AI Fundamentals - Lesson 2")
    
    def test_execute_with_course_name_filter(self):
        """Test search with course name filter"""
        mock_results = SearchResults(
            documents=["MCP specific content"],
            metadata=[{"course_title": "MCP: Build Rich-Context AI Apps", "lesson_number": 3}],
            distances=[0.15],
            error=None
        )
        ...
        self.assertEqual(self.search_tool.last_sources[0]["link"], "https://example.com/lesson3")
    ...
    def test_execute_with_lesson_number_filter(self):
        ...
        result = self.search_tool.execute("introduction", lesson_number=1)
    ...
    def test_execute_with_both_filters(self):
        ...
        result = self.search_tool.execute("advanced", course_name="Advanced AI", lesson_number=2)
    ...
    def test_execute_no_results_with_both_filters(self):
        ...
        result = self.search_tool.execute("test", course_name="Test Course", lesson_number=3)
        
        # Verify both filters are mentioned in message
        self.assertEqual(result, "No relevant content found in course 'Test Course' in lesson 3.")
